    ...
    @synchronized(swizzledClassesDictionary()){
        if (key){
            ...
            if (mode == RSSwizzleModeOncePerClass) {
                ...
                Method origMethod = class_getInstanceMethod(classToSwizzle, selector);
                if (!origMethod) {
                    //当前函数在继承链中未找到 - 错误的selector
                    return NO;
                }
                if (class_addMethod(classToSwizzle,
                                    selector,
                                    class_getMethodImplementation(classToSwizzle, selector),
                                    method_getTypeEncoding(origMethod))) {
                    //当前类未实现origin selector，如果父类已经swizzle，则return
                    for (Class currentClass = classToSwizzle;
                         nil != currentClass;
                         currentClass = class_getSuperclass(currentClass))
                    {
